...
void get_player_name(Jugador *jugador) {
  FILE *file = fopen("nombre.txt", "r");
  if (file == NULL) {
    printf("Error al abrir el archivo nombre.txt.");
    exit(1);
  }

  fgets(jugador->nombre, sizeof(jugador->nombre), file);

  while (strlen(jugador->nombre) == 0) {
    printf("Archivo nombre.txt está vacío. Ingrese un nombre: ");
...
int main() {
  ...
  while (1) {
    ...
    sleep(0.2); // Esperar 0.2 segundos (actualizar 5 veces por segundo)
  }
